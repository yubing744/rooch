...
function SignMessageCard() {
  ...
  const binaryMessage = new Uint8Array([72, 101, 108, 108, 111]); // "Hello" in ASCII
  const hexMessage = '0x' + Array.from(binaryMessage).map(b => b.toString(16).padStart(2, '0')).join('');
  ...
  return (
    <Card size="small" title="Sign Message" style={{ width: 300, margin: 10 }}>
      ...
      <Button
        style={{ marginTop: 10 }}
        onClick={async () => {
          const signature = await (window as any).unisat.signMessage(hexMessage);
